...
class IRSFHLog:
    ...
    def select_arm(self, t: int, remaining_budget: float) -> int:
        ...
        safe_rewards = np.maximum(-1/self.M + 1e-9, future_expected_rewards)
        
        log_utility = np.log1p(safe_rewards)
        ratios = np.divide(log_utility, self.c, out=np.full_like(self.c, -np.inf), where=self.c > 0)
