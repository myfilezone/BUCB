        ...
        for i in range(self.n_arms):
             ...
             if cost_i > 0 and len(arm_rewards_sequences[i]) > 0:
                 ...
                 if num_pulls > 0:
                     ...
                     total_mean_reward = np.sum(arm_rewards_sequences[i][:num_pulls])
                     
                     current_val_per_cost = total_mean_reward / (num_pulls * self.c[i])
                     if current_val_per_cost > best_val:
                         best_val = current_val_per_cost
                         best_arm = i
        return best_arm
