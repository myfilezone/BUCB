    ...
    def select_arm(self, t: int, remaining_budget: float) -> int:
        ...
        for i in range(self.n_arms):
            ...
            if self.pulls[i] == 0:
                ...
                sum_sq_diff = self.sum_x_sq[i] - 2 * mean_x * self.sum_x[i] + n * mean_x**2
        ...
        safe_sampled_means = np.maximum(-1/self.M + 1e-9, sampled_means)
        
        utility_estimates = np.log1p(safe_sampled_means)
        ...
        ratios = np.divide(utility_estimates, self.c, out=np.full_like(self.c, -np.inf), where=self.c > 0)
