...
class BUCB:
    ...
    def select_arm(self, t: int, remaining_budget: float) -> int:
        ...
        u_bar = self.u_hat + np.sqrt(2 * np.log(self.t) / (T_safe * self.M))
    ...
    def update(self, arm_index: int, reward: float, cost: float):
        ...
        self.u_hat[arm_index] = (self.T[arm_index] * self.u_hat[arm_index] + reward) / ((self.T[arm_index] + 1))
