...
class UCBG:
    ...
    def select_arm(self, t: int, remaining_budget: float) -> int:
        ...
        empirical_ratio = np.divide(self.u_hat, self.c, out=np.full_like(self.c, 0), where=self.c!=0)

        reward_range = 1.0
        cost_range = np.max(self.c) - np.min(self.c)
        scaling_factor = reward_range / np.min(self.c) if cost_range == 0 else reward_range / cost_range
