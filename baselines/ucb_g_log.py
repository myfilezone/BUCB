...
class UCBGLog:
    def __init__(self, n_arms: int, budget: float, M: int, c: np.ndarray, is_simulation: bool = True, **kwargs):
        self.n_arms = n_arms
        self.budget = budget
        self.M = M
        self.c = c
    ...
    def select_arm(self, t: int, remaining_budget: float) -> int:
        ...
        total_reward_hat = self.u_hat * self.M
        
        total_reward_hat_safe = np.where(total_reward_hat <= 0, 1e-9, total_reward_hat)

        empirical_log_ratio = np.divide(np.log(total_reward_hat_safe), self.c, out=np.full_like(self.c, -np.inf), where=self.c!=0)

        utility_range = np.log(2) 
        min_cost = np.min(self.c[self.c > 0]) if np.any(self.c > 0) else 1
        scaling_factor = utility_range / min_cost 

        exploration_term = scaling_factor * np.sqrt(2 * np.log(self.t) / T_safe)
        
        indices = empirical_log_ratio + exploration_term
